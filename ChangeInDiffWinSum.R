...
data <- read.csv("Men'sMidSeason17-18.csv", header=TRUE, stringsAsFactors=FALSE)
...
ChangeInDiffWinSum <- data.frame(Iterations=double(), DiffWinSum=double(), stringsAsFactors=FALSE) 
...
while (convergence() == FALSE)
{
  ...
  ChangeInDiffWinSum[nrow(ChangeInDiffWinSum) + 1, "Iterations"] <- k
  ChangeInDiffWinSum[nrow(ChangeInDiffWinSum), "DiffWinSum"] <- max(abs(DiffWinSum))
  k <- k + 1
}
...
#write.csv(ChangeInDiffWinSum, file = "output.csv", row.names=FALSE, na="")

print(nrow(subset(competitions, Odds < .5))/nrow(subset(competitions)))
